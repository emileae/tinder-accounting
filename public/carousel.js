...
//         successful = true;
//         posX = this.board.clientWidth;
//         console.log("right"); // Log swipe right
//       } else if (propX < -0.25 && e.direction == Hammer.DIRECTION_LEFT) {
//         successful = true;
//         posX = - (this.board.clientWidth + this.topCard.clientWidth);
//         console.log("left"); // Log swipe left
//       } else if (propY < -0.25 && e.direction == Hammer.DIRECTION_UP) {
//         successful = true;
//         posY = - (this.board.clientHeight + this.topCard.clientHeight);
//       }
...
class Carousel {
  ...
  onPan(e) {
    // Similar setup as before but include logging for actions
    if (!this.isPanning) {
      this.isPanning = true;
      this.topCard.style.transition = null;
      let style = window.getComputedStyle(this.topCard);
    ...
    // Movement calculations as before
    let posX = e.deltaX + this.startPosX;
    let posY = e.deltaY + this.startPosX;
    let propX = e.deltaX / this.board.clientWidth;
    let propY = e.deltaY / this.board.clientHeight;
    ...
    if (e.isFinal) {
      ...
      if (propX > 0.25 && e.direction == Hammer.DIRECTION_RIGHT) {
        successful = true;
        posX = this.board.clientWidth;
        console.log(this.data[this.currentIndex].rightAction); // Log right action
      } else if (propX < -0.25 && e.direction == Hammer.DIRECTION_LEFT) {
        successful = true;
        posX = - (this.board.clientWidth + this.topCard.clientWidth);
        console.log(this.data[this.currentIndex].leftAction); // Log left action
      }
  ...
  push() {
    if (this.currentIndex < this.data.length) {
      let card = document.createElement('div');
      card.classList.add('card');
      card.textContent = this.data[this.currentIndex].Situation; // Set text as situation
      this.board.insertBefore(card, this.board.firstChild);
      this.currentIndex++;
    }
  }
...
var situations = [
  {
    "Situation": "Cloud costs are going up, should we hire a consultant to fix this or a full-time employee?",
    "leftAction": "Hire a consultant",
    "rightAction": "Hire full-time employee",
    "leftResult": "10",
    "rightResult": "-10"
  },
  {
    "Situation": "2 Cloud costs are going up, should we hire a consultant to fix this or a full-time employee?",
    "leftAction": "2 Hire a consultant",
    "rightAction": "2 Hire full-time employee",
    "leftResult": "10",
    "rightResult": "-10"
  },
  {
    "Situation": "3 Cloud costs are going up, should we hire a consultant to fix this or a full-time employee?",
    "leftAction": "3 Hire a consultant",
    "rightAction": "3 Hire full-time employee",
    "leftResult": "10",
    "rightResult": "-10"
  }
...
var board = document.querySelector('#board');
var carousel = new Carousel(board, situations);
